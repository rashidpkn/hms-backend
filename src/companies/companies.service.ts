...
export class CompaniesService {
  ...
  async create(
    ...
    if (isExist) {
      ...
      if (
        isExist.email === createCompanyDto.email ||
        isExist.email2 === createCompanyDto.email
      ) {
        conflicts.push('email');
      }
      if (
        isExist.phoneNumber === createCompanyDto.phoneNumber ||
        isExist.phoneNumber2 === createCompanyDto.phoneNumber
      ) {
        conflicts.push('phone number');
      }
      if (
        isExist.licenseNumber &&
        isExist.licenseNumber === createCompanyDto.licenseNumber
      ) {
  ...
  async findAll(): Promise<CommonResponse<{}[]>> {
    const data = await this.db.query.companiesTable.findMany({
