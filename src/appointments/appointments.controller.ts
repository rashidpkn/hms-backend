...
export class AppointmentsController {
  ...
  @Get("doctor/:id")
  async getAppointmentsByDoctor(@Param("id", ParseIntPipe) doctorId: number, @GetUser() user: AuthUser) {
    return this.appointmentsService.getAppointmentsByDoctor(doctorId, user.companyId);
  }

  @Get("patient/:id")
  async getAppointmentsByPatient(@Param("id", ParseIntPipe) patientId: number, @GetUser() user: AuthUser) {
    return this.appointmentsService.getAppointmentsByPatient(patientId, user.companyId);
  }

  @Get("company/:id")
  async getAppointmentsByCompany(@Param("id", ParseIntPipe) companyId: number, @GetUser() user: AuthUser) {
  ...
  @Get(":id")
  async getAppointmentById(@Param("id", ParseIntPipe) id: number, @GetUser() user: AuthUser) {
    return this.appointmentsService.getAppointmentById(id, user.companyId);
  }

  @Patch("reschedule/:id")
  async rescheduleAppointment(@Param("id", ParseIntPipe) id: number, @Body() rescheduleAppointmentDto: RescheduleAppointmentDto, @GetUser() user: AuthUser) {
    return this.appointmentsService.rescheduleAppointment(id, rescheduleAppointmentDto, user.companyId);
  }

  @Patch("status/:id")
  async updateAppointmentStatus(@Param("id", ParseIntPipe) id: number, @Body() body: ChangeStatusDto, @GetUser() user: AuthUser) {
    return this.appointmentsService.updateAppointmentStatus(id, body.status, user.companyId);
  }

  @Delete(":id")
  async deleteAppointment(@Param("id", ParseIntPipe) id: number, @GetUser() user: AuthUser) {
